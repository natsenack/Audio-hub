#!/usr/bin/env python3
"""Indicateur de barre d'état pour AudioHub via AppIndicator."""
...
APP_ICON_NAME = "audio-hub"
APP_NAME = "AudioHub"


def _load_indicator_namespace() -> bool:
    """Vérifie si AppIndicator ou AyatanaAppIndicator sont disponibles."""
    try:
...
def _resolve_launcher_command() -> Optional[list[str]]:
    """Trouve le chemin du lanceur audio-hub."""
    ...
    launcher_path = shutil.which("audio-hub")
...
def _run_launcher(arguments: list[str]) -> None:
    """Exécute le lanceur avec les arguments donnés."""
    try:
...
def _create_menu() -> Gtk.Menu:
    """Crée le menu contextuel de la barre d'état."""
    menu = Gtk.Menu()

    open_item = Gtk.MenuItem.new_with_label("Afficher AudioHub")
    open_item.connect("activate", lambda *_args: _launch_show())
    menu.append(open_item)

    quit_item = Gtk.MenuItem.new_with_label("Quitter AudioHub")
...
def _launch_show() -> None:
    """Lance AudioHub (ou montre la fenêtre si déjà lancé)."""
...
def _configure_icon_theme() -> None:
    """Configure le thème d'icônes pour trouver l'icône d'AudioHub."""
...
def main() -> int:
    """Lance l'indicateur de barre d'état."""
